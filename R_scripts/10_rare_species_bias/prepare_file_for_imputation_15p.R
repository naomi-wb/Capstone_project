...
detectability <- read_csv("Intermediate data files/spp_all_15p.csv") %>%
...
imputation_file <- clements_clean %>%
  ...
  dplyr::select(1:8, 13:18) %>%       #only select specific columns
  ungroup() %>%
...
saveRDS(joined_dat_final, "Intermediate data files/file_for_imputation_15p.RDS")
...
ggsave("Figures/Supplementary_figures/intercept_and_slope_trait_relationships_15p.png", 
...
ggsave("Figures/Supplementary_figures/density_and_rel_abund_trait_relationships_15p.png", 
